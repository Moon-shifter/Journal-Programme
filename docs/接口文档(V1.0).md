...
最后更新：12.24 **说明：教师的主键取消自增，期刊表的ISBN改成ISSN。请将数据库修改与下方一致（随时可能改动）.前端请求体字段务必与api文档一致**



技术说明：数据格式：JSON；请求方式：GET/POST/PUT/DELETE；认证方式：Session 
...
3.**数据库规范：**(请在sqlyog或idea做好相应调整,自行搜索了解主键（唯一）等概念，注意默认值)

- 管理员表：主键为admin_id,自动递增，从哪里开始递增请自行搜索设定，不会影响功能
  注意下方的默认值（**后面跟上了=**），有默认值的地方在插入时有时是不用管的，见请求体格式要求
...
- 期刊表：主键id，无自增栏目,下方status默认为available
  ![loading-ag-1172](pictures/055a53ad-0988-4fee-9698-3806434a51f8.png)
  
  

- 教师表：主键为teacher_id不自增， status默认inactive(同管理员)
  ![loading-ag-1209](pictures/eeaca58f-ea52-4c17-99fd-69d7ef3fc340.png)
...
- **请求体**（对应`AdminInfo`实体）：
  ...
  | password | String | 是    | 管理员密码 | 8-16 位，可含数字 + 字母+特殊符号 |
...
* **请求体**（对应`JournalInfo`实体,`id`为必填标识）：
  ...
  | issn              | String    | 否    | ISSN 号 | 长度 1-20,唯一                  |
...
* **路径**：`/journal/admin/add`

* **方法**：POST

* **请求体**（对应`JournalInfoDTO`实体）：
  
  | 字段名               | 类型        | 是否必填 | 说明     | 约束条件                                       |
  | ----------------- | --------- | ---- | ------ | ------------------------------------------ |
  | id                | Integer   | 是    | 期刊id   | 纯数字，长度有限                                   |
  | name              | String    | 是    | 期刊名称   | 长度 1-100                                   |
  | issn              | String    | 是    | ISSN 号 | 长度 8-20，符合 ISSN 格式，唯一                      |
  | category          | String    | 是    | 期刊类别   | 枚举：“计算机”“数学”“文学” 等                         |
  | publisher         | String    | 是    | 出版社    | 长度 1-100                                   |
  | publishDate       | LocalDate | 否    | 出版日期   | ISO 格式：“2025-06-01” 默认值"1970-1-1"          |
  | issueNumber       | String    | 是    | 期号     | 如 “2025 年第 3 期”，长度 1-20                    |
  | description       | String    | 是    | 期刊描述   | 长文本，不能有默认值                                 |
  | totalQuantity     | Integer   | 否    | 总数量    | 大于 0，如 5/10/20，默认值0                        |
  | availableQuantity | Integer   | 否    | 可获取数量  | 小于等于总数量且大于0，默认值0                           |
...
| V1.1 | 2025-12-23 | 补充规范化了教师管理后端响应体，使其与api文档符合 | 伍怀进 |
| V1.2 | 2025-12-24 | 优化了模块一二的接口，现在可以正常使用了       | 伍怀进 |
